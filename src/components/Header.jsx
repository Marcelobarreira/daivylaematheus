    ...
    <header className={`absolute top-0 left-0 w-full z-50 ${withBackground ? 'bg-[#7A7A7A]' : 'bg-transparent'}`}>
      ...
      {menuOpen && (
        <div className="md:hidden bg-gradient-to-b from-black/80 to-black/40 backdrop-blur-md text-white flex flex-col items-center gap-4 py-6 px-6 text-lg font-medium transition-all duration-300">
          <a href="/" className="hover:opacity-80 transition" onClick={() => setMenuOpen(false)}>Início</a>
          <a href="/presentes" className="hover:opacity-80 transition" onClick={() => setMenuOpen(false)}>Presentes</a>
          <a href="/informacoes" className="hover:opacity-80 transition" onClick={() => setMenuOpen(false)}>Informações</a>
          <a
            href="#rsvp"
